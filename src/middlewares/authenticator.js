...
async function checkPermissions(roleTitle, requestPath, requestMethod) {
  ...
  if (parts[4])
    api_path.push(`/${parts[0]}/${parts[1]}/${parts[2]}/${parts[3]}*`);
  else
    api_path.push(
  ...
  const allowedPermissions = await rolePermissionMappingQueries.findAll(
    filter,
    attributes
  );
...
module.exports = async function (req, res, next) {
  ...
  try {
    ...
    if (decodedToken.data.roles) {
      isAdmin = decodedToken.data.roles.some(
        (role) => role.title == common.ADMIN_ROLE
      );
      if (isAdmin) {

...
module.exports = class EntityHelper {
  ...
  static async update(bodyData, id, loggedInUserId, orgId) {
    ...
    try {
      const [updateCount, updatedEntityType] =
        await entityTypeQueries.updateOneEntityType(id, orgId, bodyData, {
          returning: true,
          raw: true,
        });
  ...
  static async readUserEntityTypes(body, userId, orgId) {
    try {
      ...
      const entities = await entityTypeQueries.findUserEntityTypesAndEntities(
        filter
      );
  ...
  static async delete(id, orgId) {
    try {
      const deleteCount = await entityTypeQueries.deleteOneEntityType(
        id,
        orgId
      );
      if (deleteCount === 0) {

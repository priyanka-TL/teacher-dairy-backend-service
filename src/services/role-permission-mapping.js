...
module.exports = class modulesHelper {
  ...
  static async create(roleTitle, permissionId, id) {
    try {
      const permission = await permissionsQueries.findPermissionId(
        permissionId
      );
      if (!permission) {
      ...
      const rolePermissionMapping = await rolePermissionMappingQueries.create(
        data
      );
  ...
  static async delete(roleTitle, permissionId) {
    try {
      ...
      const rolePermissionMapping = await rolePermissionMappingQueries.delete(
        filter
      );
  ...
  static async list(roleTitles) {
    try {
      ...
      const permissionAndModules = await rolePermissionMappingQueries.findAll(
        filter,
        attributes
      );
      ...
      const permissions = Object.values(permissionsByModule).map(
        ({ module, request_type }) => ({
          module,
          request_type,
